...
public class ActivityService {
    ...
    private void setAct(Activity act,Account currentUser){
        act.setIsEnd(DateUtil.parseDate(act.getEnd()).isBefore(new Date())); //活动的结束时间在当前时间之前就表示活动结束了
    ...
    public PageInfo<Activity> selectUser(Activity activity, Integer pageNum, Integer pageSize) {
        ...
        List<Activity> collect = list.stream().filter(Objects::nonNull).collect(Collectors.toList());
        PageInfo<Activity> pageInfo = PageInfo.of(collect);
        List<Activity> activityList = pageInfo.getList();
        for (Activity act : activityList) {
            if (act!=null ){
                this.setAct(act,currentUser);
